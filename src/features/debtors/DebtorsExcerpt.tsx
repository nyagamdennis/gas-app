...
const DebtorsExcerpt = ({ debtor, onClearClick, onCloseClick }) => {
  ...
  return (
    <TableRow key={debtor.id} hover role="checkbox" tabIndex={-1}>
      ...
      {/* <TableCell>{debtor?.sales_tab?.quantity} {debtor?.sales_tab?.product?.gas_type?.name} {debtor?.sales_tab?.product?.cylinder?.weight?.weight}Kg</TableCell> */}
      ...
      <TableCell>
        <div className=' flex space-x-1 items-center'>
          ...
          <button onClick={() => onClearClick({debtor, customer})} className='bg-green-800 text-white px-2 py-0.5 rounded-md'>Clear</button>
