...
const Expenses = () => {
  ...
  const handleClickUpdateOpen = (expenseId:string, expenseNames:string, expenseAmount: number) => {
  ...
  const handleEdit = async (e:any) => {
    ...
    try {
      await dispatch(updateCompanyExpense({expenseId: updateExpenseId, formData}))
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      {/* Command bar */}
      <div className="bg-white shadow p-4 flex justify-end items-center border-b border-gray-200">
            ...
            <tbody className="divide-y divide-gray-100">
              {all_expenses.map((expense) => (
                <tr key={expense.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3">{expense.expense_name}</td>
                  ...
                  <td className="px-4 py-3 space-x-2">
                    <button
                      onClick={() => handleClickUpdateOpen(expense.id, expense.expense_name, expense.amount)}
                      className="text-yellow-600 hover:underline"
                    >
                      Edit
                    </button>
                    <button
                      onClick={() => handleClickDeleteOpen(expense.id, expense.expense_name)}
            ...
            <form onSubmit={handleEdit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e: any) => setUpdateExpenseAmount(e.target.value)}
                  className="mt-1 block w-full border border-neutral-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary sm:text-sm"
                  placeholder="Enter amount"
                />
              </div>
