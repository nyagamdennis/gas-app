...
// Add/edit entries here as you add more plans
const PLAN_CONFIG: Record<
  string,
  { label: string; emoji: string; mobile: string; desktop: string }
> = {
  Basic: {
    label: "Basic",
    emoji: "📦",
    mobile: "bg-sky-100 text-sky-700",
    desktop: "bg-sky-100 text-sky-700",
  },
  Pro: {
    label: "Pro",
    emoji: "✨",
    mobile: "bg-violet-100 text-violet-700",
    desktop: "bg-violet-100 text-violet-700",
  },
  Business: {
    label: "Business",
    emoji: "🏢",
  ...
  EXPIRED: {
    ...
    desktop: "bg-red-100 text-red-600",
  },
  default: {
    label: "Free Plan",
    emoji: "🆓",
...
const getPlanConfig = (
  ...
  if (isExpired) return PLAN_CONFIG["EXPIRED"]
  if (status === "TRIAL") return PLAN_CONFIG["TRIAL"]
  if (planCode && PLAN_CONFIG[planCode]) return PLAN_CONFIG[planCode]
  return PLAN_CONFIG["default"]
}
...
const desktopMenuItems = [
  ...
  },
  {
    ...
    subtitle: "Manage and view all receipts",
    gradient: "from-sky-500 to-sky-600",
  },
]

// ─── Plan Badge component ──────────────────────────────────────────────────────
const PlanBadge = ({ size = "md" }: { size?: "sm" | "md" }) => {
  const subscriptionPlan = useAppSelector(selectSubscriptionPlan)
  const subscriptionPlanCode = useAppSelector(selectSubscriptionPlanCode)
  ...
  const cfg = getPlanConfig(subscriptionPlanCode, subscriptionStatus, isExpired)

  // Show expiry info if not expired and has a date
  const expiryLabel =
    !isExpired && expiresAt
      ? ` · expires ${new Date(expiresAt).toLocaleDateString("en-KE", {
          day: "numeric",
          month: "short",
        })}`
      : ""
  ...
  if (size === "sm") {
    return (
      <span
        className={`px-3 py-1 rounded-full text-xs font-semibold ${cfg.mobile}`}
      >
        {cfg.emoji} {subscriptionPlan || cfg.label}
        {expiryLabel}
      </span>
    )
  }
  ...
  return (
    <span
      className={`px-4 py-2 rounded-full text-sm font-semibold ${cfg.desktop}`}
    >
      {cfg.emoji} {subscriptionPlan || cfg.label}
      {expiryLabel}
    </span>
...
const AdminHome = () => {
  ...
  // Business name from planStatusSlice (decoded from JWT)
